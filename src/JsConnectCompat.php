...
/**
 * This class contains backwards compatible methods for the v2 jsConnect protocol.
 */
final class JsConnectCompat {
    const JS_CONNECT_VERSION = '2';
    const JS_TIMEOUT = 24 * 60;
    ...
    public static function writeJsConnect($user, $request, $clientID, $secret, $secure = true) {
        ...
        if ($secure) {
            ...
            elseif (abs($request['timestamp'] - self::jsTimestamp()) > self::JS_TIMEOUT) {
                ...
                $signature = self::jsHash($request['ip'] . $request['nonce'] . $request['timestamp'] . $secret, $secure);
        ...
        if (!headers_sent()) {
            $contentType = self::jsConnectContentType($request);
    ...
    public static function signJsConnect($data, $clientID, $secret, $hashType, $returnData = false) {
        ...
        $signature = self::jsHash($stringifiedData . $secret, $hashType);
        if ($returnData) {
     ...
     * @return string
     */
    public static function jsConnectContentType(array $request): string {
