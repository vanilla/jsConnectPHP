...
class JsConnectServer extends JsConnect
{
    ...
    public function generateRequest(array $state = []): array
    {
        if (isset($state[self::FIELD_COOKIE])) {
            ...
            $nonce = self::validateFieldExists(
                self::FIELD_NONCE,
                $decodedCookie,
                "ssoToken"
            );
        ...
        $requestUrl =
            $this->getAuthenticateUrlWithSeparator() .
            http_build_query(["jwt" => $requestJWT]);
    ...
    public function validateResponse(?string $jwt, ?string $cookieJWT): array
    {
        ...
        $cookieNonce = static::validateFieldExists(
            static::FIELD_NONCE,
            $cookie,
            "cookie"
        );
        $stateNonce = static::validateFieldExists(
            static::FIELD_NONCE,
            $state,
            "state"
        );
    ...
    {
        return $this->authenticateUrl .
            (strpos($this->authenticateUrl, "?") === false ? "?" : "&");
    }
